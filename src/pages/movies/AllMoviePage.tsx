...
function AllMoviePage() {
  ...
  const [allMovieList, setAllMovieList] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (movieData?.data) {
      if (selectedCategory) {
        ...
        setAllMovieList(filteredLives);
      } else {
        setAllMovieList(movieData.data);
      }
    }
  }, [movieData, selectedCategory]);

  // Filtered + paginated movies
  const filteredMovies = movieSearchText
    ? allMovieList.filter((movie) =>
        movie?.name?.toLowerCase().includes(movieSearchText.toLowerCase())
      )
    : allMovieList;
